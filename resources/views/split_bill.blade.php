...
<head>
    ...
    <style>
        ...
        .font-numbers { font-family: 'JetBrains Mono', monospace; letter-spacing: -0.5px; }
        ...
        /* Inputs */
        .input-clean {
            background: #fff; border: 1px solid #CBD5E1; color: #1E293B; transition: all 0.2s;
        }
        .input-clean:focus {
        ...
        /* Voucher Logic */
        #voucher-list { display: flex; flex-direction: column; position: relative; }
        .voucher-card { 
        ...
        .voucher-card.best-pick .best-label { display: inline-block; }
        ...
        @media print {
            ...
            #receipt-modal { position: absolute; left: 0; top: 0; width: 100%; height: 100%; background: white; z-index: 9999; display: flex !important; justify-content: center; align-items: flex-start; }
            #receipt-scroll-container { max-height: none; overflow: visible; box-shadow: none; }
            #receipt-wrapper { width: 58mm; margin: 0; border: none; }
...
<body class="min-h-screen pb-12 bg-[url('https://grainy-gradients.vercel.app/noise.svg')]">
    ...
    <nav class="fixed top-0 w-full z-40 bg-white/90 backdrop-blur-md border-b border-slate-200">
        <div class="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
            <div class="flex items-center gap-3">
                <div class="bg-indigo-600 text-white w-10 h-10 rounded-xl flex items-center justify-center text-lg shadow-lg shadow-indigo-200">
            ...
            <div class="hidden md:flex items-center gap-2 text-xs font-semibold bg-slate-100 px-4 py-2 rounded-full border border-slate-200 text-slate-600">
                <i class="fa-solid fa-code-branch"></i> Studi Iteratif vs Rekursif pada Algoritma 0/1 Knapsack
            </div>
            <button onclick="resetData()" class="text-xs font-bold text-slate-400 hover:text-red-500 transition uppercase tracking-wider flex items-center gap-2">
        ...
        <div class="lg:col-span-7 space-y-6">
            ...
            <div class="glass-panel rounded-3xl p-6 md:p-8">
                <div class="flex justify-between items-center mb-6">
                    <h2 class="text-lg font-bold text-slate-800 flex items-center gap-2">
                        <span class="flex items-center justify-center w-7 h-7 rounded-full bg-slate-800 text-white text-xs">1</span>
                        Produk & Harga
                    </h2>
            ...
            <div class="glass-panel rounded-3xl p-6 md:p-8">
                <h2 class="text-lg font-bold text-slate-800 mb-2 flex items-center gap-3">
                    <span class="flex items-center justify-center w-7 h-7 rounded-full bg-slate-800 text-white text-xs">2</span>
                    Anggota
                </h2>
                ...
                <button onclick="addMember()" class="w-full py-3 border-2 border-dashed border-slate-300 rounded-xl text-slate-400 font-bold hover:text-indigo-600 hover:border-indigo-400 transition">
                    + Tambah Anggota
                </button>
            </div>
            ...
            <div class="glass-panel rounded-3xl p-6 md:p-8">
                <h2 class="text-lg font-bold text-slate-800 mb-6 flex items-center gap-3">
                    <span class="flex items-center justify-center w-7 h-7 rounded-full bg-slate-800 text-white text-xs">3</span>
                    Voucher & Pajak
                </h2>
                ...
                <div class="grid grid-cols-2 gap-6 mb-8">
                    <div>
                        ...
                        <input type="number" id="tax-input" class="input-clean w-full rounded-xl px-4 py-3 font-bold font-numbers" value="10" oninput="saveLocal()">
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-slate-500 uppercase mb-2">Service (Rp)</label>
                        <input type="text" id="service-input" class="input-clean w-full rounded-xl px-4 py-3 font-bold font-numbers" placeholder="0" onkeyup="formatRupiah(this); saveLocal()">
                    </div>
                </div>
                ...
                <div class="bg-slate-50 p-5 rounded-2xl border border-slate-200">
                    <div class="flex justify-between items-center mb-4">
                        ...
                        <button onclick="openVoucherManager()" class="text-xs bg-white border border-slate-300 px-3 py-1.5 rounded-lg font-bold text-indigo-600 hover:shadow-sm">
            ...
            <button onclick="generateBill()" class="w-full bg-slate-900 hover:bg-indigo-600 text-white font-bold text-lg py-5 rounded-2xl shadow-xl transition-all transform active:scale-[0.98] flex justify-center items-center gap-3 group">
            ...
            <div id="results-area" class="hidden space-y-6 fade-in">
                ...
                <div class="bg-white rounded-3xl overflow-hidden shadow-2xl border border-slate-100">
                    <div class="bg-gradient-to-br from-slate-800 to-slate-900 p-8 text-white relative">
                        <div class="absolute right-0 top-0 p-6 opacity-10 text-8xl"><i class="fa-solid fa-receipt"></i></div>
                        <p class="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">Total Tagihan Final</p>
                    ...
                    <div class="p-6 bg-white">
                        <div class="flex justify-between items-center mb-4">
                            ...
                            <button onclick="showReceiptModal()" class="text-xs font-bold text-indigo-600 hover:text-indigo-800 flex items-center gap-1">
                ...
                <div class="glass-panel rounded-3xl p-6">
                    ...
                    <div class="space-y-4 text-xs text-slate-600 bg-slate-50 p-4 rounded-xl border border-slate-200 mb-6 leading-relaxed">
                        <div>
                        ...
                        <div>
                            <p class="font-bold text-indigo-600 uppercase mb-1">2. Relasi Rekurens (Dynamic Programming)</p>
                            <p class="mb-1">Kami menggunakan pendekatan <em>Bottom-Up</em> untuk mengisi tabel $K[i][w]$:</p>
                            <div class="bg-white p-2 rounded border border-slate-200 font-numbers text-[10px] overflow-x-auto">
                                if (wt[i] <= w): <br>
                        ...
                        <div>
                            ...
                            <p>
                                Algoritma berjalan dalam waktu <strong>Pseudo-Polynomial</strong>. Meskipun efisien untuk N kecil, kompleksitas bergantung linear pada kapasitas W (Total Belanja). Kami membandingkan pendekatan <strong>Iteratif</strong> (Tabulasi) dan <strong>Rekursif</strong> (Memoization) di bawah ini.
                            </p>
                        </div>
                    </div>

                    <h4 class="font-bold text-xs text-slate-400 uppercase mb-3">Grafik Benchmark Runtime (Line Chart)</h4>
                    ...
                    <div class="flex justify-between text-xs font-bold text-slate-500 px-1">
                        <span class="text-indigo-600">Iteratif: <span id="time-iter" class="font-numbers">0ms</span></span>
                        <span class="text-pink-600">Rekursif: <span id="time-rec" class="font-numbers">0ms</span></span>
                    </div>
                </div>

            </div>

            <div id="empty-state" class="h-full min-h-[500px] glass-panel rounded-3xl flex flex-col items-center justify-center text-center p-10 border-2 border-dashed border-slate-300">
                ...
                <p class="text-xs text-slate-400 mt-1 max-w-xs">Masukkan produk, anggota, dan voucher untuk memulai analisis algoritma.</p>
            </div>
        </div>
    </main>

    <div id="manage-voucher-modal" class="hidden fixed inset-0 z-[80] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
        <div class="bg-white w-full max-w-2xl rounded-3xl p-6 shadow-2xl relative">
            <h3 class="text-lg font-bold text-slate-800 mb-4 flex items-center gap-2"><i class="fa-solid fa-ticket text-indigo-600"></i> Kelola Daftar Voucher</h3>
            ...
            <button onclick="addVoucherRowModal()" class="w-full py-3 border-2 border-dashed border-slate-300 rounded-xl text-slate-400 font-bold hover:text-indigo-600 hover:border-indigo-400 transition mb-4 text-sm">
                + Tambah Voucher Baru
            </button>
    ...
    <div id="success-modal" class="hidden fixed inset-0 z-[60] flex items-center justify-center px-4">
        ...
        <div class="relative bg-white w-full max-w-sm rounded-3xl p-8 shadow-2xl scale-100 transition-transform">
            <div class="text-center">
                <div class="w-16 h-16 bg-emerald-100 text-emerald-600 rounded-full flex items-center justify-center mx-auto mb-4 text-3xl shadow-lg shadow-emerald-50">
                ...
                <button onclick="closeModal()" class="w-full bg-slate-900 text-white font-bold py-3 rounded-xl hover:bg-slate-800 transition">Lihat Hasil</button>
            </div>
        </div>
    </div>

    <div id="receipt-modal" class="hidden fixed inset-0 z-[70] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
        <div class="bg-white w-full max-w-sm shadow-2xl relative overflow-hidden flex flex-col max-h-[90vh]" id="receipt-container">
            <div id="receipt-scroll-container">
                <div id="receipt-wrapper" class="p-8 font-mono text-sm text-slate-900 bg-[url('https://grainy-gradients.vercel.app/noise.svg')] bg-cover">
                    ...
                    <div id="rec-items" class="space-y-2 mb-4 pb-4 border-b-2 border-dashed border-slate-400/50 text-xs"></div>
                    ...
                    <div class="space-y-1 text-right mb-4 pb-4 border-b-2 border-dashed border-slate-400/50 text-xs">
                        ...
                        <div class="flex justify-between font-bold text-emerald-600"><span>Voucher <span id="rec-voucher-code" class="text-[9px] font-normal text-slate-500"></span></span><span id="rec-disc">0</span></div>
                    </div>
                    ...
                    <div class="text-center mb-3">
                        <div class="inline-block bg-slate-900 text-white text-[9px] font-bold px-2 py-1 rounded uppercase tracking-wider">Split Bill Details</div>
                    </div>
    ...
    <script>
        ...
        let membersData = [];
        let vouchers = [
            {code: "HEMAT10", discount: "10.000", min_spend: "50.000", qty: 5},
            {code: "DISKON50", discount: "50.000", min_spend: "200.000", qty: 2},
            {code: "NAT4L", discount: "15.000", min_spend: "5.000", qty: 10}
        ];
        let lastResult = null;
        ...
        function updateVoucherRanking(currentTotal) {
            ...
            vouchers.forEach((v, idx) => {
                ...
                if(currentTotal >= min && currentTotal > 0) {
                    scoredVouchers.push({ index: idx, val: val });
                }
            });
        ...
        function renderVouchersMain(scoredVouchers = []) {
            ...
            let rankMap = {};
            scoredVouchers.forEach((item, rank) => { rankMap[item.index] = rank; });
        ...
        function addVoucherRowModal() {
            vouchers.push({code:"", discount:"", min_spend:"", qty: 1});
            renderVoucherManager();
            saveLocal();
        }
        ...
        function addMember(name='') {
            membersData.push({ id: 'mem_'+Date.now(), name: name, selections: {} });
            renderMembersUI();
            saveLocal();
        }
        ...
        function renderMembersUI() {
            ...
            membersData.forEach((mem, mIdx) => {
                ...
                if(globalItems.length === 0) list.innerHTML = '<span class="text-xs text-slate-400 italic">Belum ada produk.</span>';
                
                globalItems.forEach(item => {
                    if(!item.name) return;
                    let taken = 0;
                    membersData.forEach((om, oi) => { if(oi !== mIdx) taken += (om.selections[item.id] || 0); });
                    ...
                    row.className = `flex justify-between items-center p-2 rounded-lg text-xs ${myQty > 0 ? 'bg-indigo-50' : ''}`;
                    row.innerHTML = `
        ...
        async function generateBill() {
            ...
            try {
                const res = await fetch('/calculate', {
                    ...
                    body: JSON.stringify({ items: globalItems, members: membersData, vouchers: vouchers, tax: document.getElementById('tax-input').value, service: document.getElementById('service-input').value })
                });
        ...
        function showReceiptModal() {
            ...
            const ib = document.getElementById('rec-items'); ib.innerHTML = '';
            globalItems.forEach(i => { ib.innerHTML += `<div class="flex justify-between"><span>${i.name} x${i.qty}</span><span>${i.price}</span></div>`; });
            ...
            const vCode = (lastResult.summary.used_vouchers && lastResult.summary.used_vouchers.length > 0) ? lastResult.summary.used_vouchers[0] : "";
            ...
            const sb = document.getElementById('rec-splits'); sb.innerHTML = '';
        ...
        function saveLocal() {
            const d = { globalItems, membersData, vouchers, tax: document.getElementById('tax-input').value, svc: document.getElementById('service-input').value };
        ...
        function loadLocal() {
            ...
            if(raw) {
                ...
                if(d.globalItems && d.globalItems.length > 0) {
                    const tbody = document.getElementById('product-list');
                    tbody.innerHTML = ''; 
                    d.globalItems.forEach(item => { addProductRow(item.name, item.qty, item.price); });
                }
        ...
        function resetData() {
            if(confirm("Hapus Data Produk & Anggota?")) {
                globalItems = []; membersData = []; saveLocal(); location.reload();
            }
        }
        ...
        function showModal(used) {
            const c = document.getElementById('popup-vouchers'); c.innerHTML = '';
            if(used.length > 0) used.forEach(code => c.innerHTML += `<span class="bg-emerald-100 text-emerald-700 px-3 py-1 rounded-full text-sm font-bold font-numbers border border-emerald-200">${code.toUpperCase()}</span>`);
        ...
        function renderChart(d) {
            ...
            const animation = {
                x: { type: 'number', easing: 'linear', duration: delayBetweenPoints, from: NaN, delay(ctx) { return ctx.type !== 'data' || ctx.xStarted ? 0 : ctx.index * delayBetweenPoints; }, fn(ctx) { if (ctx.type === 'data') ctx.xStarted = true; } },
                y: { type: 'number', easing: 'linear', duration: delayBetweenPoints, from: previousY, delay(ctx) { return ctx.type !== 'data' || ctx.yStarted ? 0 : ctx.index * delayBetweenPoints; }, fn(ctx) { if (ctx.type === 'data') ctx.yStarted = true; } }
            };
            myChart = new Chart(ctx, { type: 'line', data: { labels: d.labels.map(l => 'N='+l), datasets: [ { label: 'Iteratif', data: d.iterative, borderColor: '#4F46E5', backgroundColor: 'rgba(79, 70, 229, 0.1)', fill: true, tension: 0.3, pointRadius: 4 }, { label: 'Rekursif', data: d.recursive, borderColor: '#EC4899', borderDash: [5,5], tension: 0.3, pointRadius: 4 } ] }, options: { animation, responsive: true, maintainAspectRatio: false, plugins: { legend: { labels: { font: { family: 'Poppins' } } } }, scales: { x: { grid: { display: false }, ticks: { font: { family: 'JetBrains Mono' } } }, y: { grid: { color: '#F1F5F9' }, ticks: { font: { family: 'JetBrains Mono' } } } } } });
        }
    </script>
    <footer class="mt-12 pb-4 text-center">
    <div class="inline-flex items-center gap-2 px-6 py-3 bg-white/50 backdrop-blur-sm rounded-full border border-slate-200 shadow-sm transition-all hover:shadow-md">
